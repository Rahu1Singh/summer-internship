...
const Header = () => {
	return (
	  	<section id="header" className='relative w-full h-[200px] xl:h-[460px] p-10 flex items-center lg:mb-10 border-b-[20px] border-b-800 rounded-bl-3xl rounded-br-3xl' style={{backgroundImage: "url(https://images.unsplash.com/photo-1517048676732-d65bc937f952?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)", backgroundPosition:'0% 45%'}}>
...
const Intro = () => {
	return (
	  <div className="flex flex-col md:flex-row justify-center mt-10 py-10 md:w-3/4 mx-auto">
			<div className="md:w-2/5 mx-10 my-5 font-semibold text-xl text-b-900 xl:text-3xl md:text-right">SERVICES <span className="text-black">PROVIDED <br className="hidden md:block" />AT INGENIUM</span></div>
			<div className="w-[4px] bg-black" aria-hidden="true" />
			<p className="md:w-3/5 mx-10 flex items-center text-[17px]">Ingenium Automation is dedicated to providing exceptional services that drive success for our clients across industries. We provide Design/Enginerring/Programming/Graphic development/Supply/Installation/Testing/Commissioning and Annual Maintenance Contract for the above given systems.  Contact us to learn more about how we can partner with you to achieve your automation goals.</p>
  		</div>
  	)
}
...
const ServiceCard = ({children, img}) => {
  return (
    	<div className="relative my-10 py-10">
    		<div className="relative xl:w-4/5 xl:h-[536px] z-10 mx-auto xl:bg-b-200 xl:rounded-3xl p-10 flex flex-row items-center">
				<div className="hidden 2xl:block relative m-10">
					...
					<div className="absolute top-[20px] left-[20px] w-[400px] h-[400px] bg-b-700 rounded-3xl" aria-hidden="true" />
				</div>
				{children}
			</div>
			<div className="hidden xl:block absolute w-4/5 h-[536px] top-[12%] left-[12%] bg-b-700 rounded-3xl" aria-hidden="true" />
    	</div>
  	)
}
...
const AutomationContent = () => {
	return (
		<div className="relative flex flex-col text-left lg:m-10">
...
const EngineeringContent = () => {
	return (
		<div className="relative flex flex-col text-left p-2 lg:m-10">
